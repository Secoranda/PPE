...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static int timer_speed = 2, nrObj = 0;
	static RECT space_dimensions = { 0, 0, 700, 500 };
	...
	case WM_PAINT:
	{
		...
		{
			OBJ[i]->OBJ_Action(hdcMem, rect, hBrush);
		}
	...
	case WM_KEYDOWN:
	{
		...
		case VK_SPACE:
		{
			...
			OBJ[nrObj] = new Circle(coord, 10 + coord.x % 2);
	...
	case WM_LBUTTONDOWN:
	{
		...
		OBJ[nrObj] = new Circle(coord, 2 + coord.x % 5);
		OBJ[nrObj]->OBJ_Color(RGB(
